...
describe('SupabaseService - Players', () => {
  ...
  it('should add a player', async () => {
    const mockSingle = vi.fn(() => ({ data: { id: 'player-id' }, error: null }));
    ...
    const playerId = await service.addPlayer('hola', 'ac798200-40df-4e6b-b2f3-e20e108ea816');
  ...
  it('should get all players', async () => {
    ...
    expect(players.map(p => p.name)).toEqual(['hola', 'adios', 'buenas', 'tardes']);
  ...
  it('should update player without affecting stats', async () => {
    ...
    await expect(service.updatePlayer('player-id', { name: 'Nuevo Nombre' })).resolves.toBeUndefined();
  ...
  it('should update player with seasonId correctly', async () => {
    ...
    await expect(service.updatePlayer('player-id', { name: 'Nuevo Nombre', seasonId: 'season-123' })).resolves.toBeUndefined();
    expect(mockUpdate).toHaveBeenCalledWith({
      name: 'Nuevo Nombre',
  ...
  it('should update player stats correctly', async () => {
    ...
    await service.updatePlayerStats('player-id', { matches: 1, wins: 1, goals: 2 }, 'ac798200-40df-4e6b-b2f3-e20e108ea816');
