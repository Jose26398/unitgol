...
describe("SupabaseService - Matches", () => {
	...
	it("should add a match", async () => {
		...
		const mockPlayerSelect = vi.fn(() => ({
			eq: vi.fn(() => ({
				single: vi.fn(() => ({
					data: { matches: 0, wins: 0, losses: 0, goals: 0, assists: 0 },
					error: null,
				})),
			})),
		}));
		...
		const match = {
			...
			goals: [
				{ playerId: "1", minute: 5 },
				{ playerId: "3", minute: 30, assistById: "2" },
			],
		};

		const matchId = await service.addMatch(match);
		expect(matchId).toBe("match-id");
		expect(mockInsertGoals).toHaveBeenCalledWith([
			{ match_id: "match-id", player_id: "1", assist_by_id: null, minute: 5 },
			{ match_id: "match-id", player_id: "3", assist_by_id: "2", minute: 30 },
		]);
	});
	...
	it("should delete match and roll back player stats", async () => {
		const mockMatchData = {
			id: "match-id",
			date: "2024-01-01",
			season_id: "ac798200-40df-4e6b-b2f3-e20e108ea816",
		...
		const mockGetMatchSingle = vi.fn(() => ({
			data: mockMatchData,
			error: null,
		}));
		...
		const mockPlayerSelect = vi.fn(() => ({
			eq: vi.fn(() => ({
				single: vi.fn(() => ({
					data: { matches: 1, wins: 1, losses: 0, goals: 0, assists: 0 },
					error: null,
				})),
			})),
		}));
		...
		// subtractMatchStats ran: each involved player's stats were rolled back
		expect(mockPlayerUpdate).toHaveBeenCalledWith(
			expect.objectContaining({ matches: 0 }),
		);
	});
	...
	it("should edit match", async () => {
		const mockCurrentMatchData = {
			...
			goals: [{ id: "g1", player_id: "1", assist_by_id: null, minute: 10 }],
		};

		const mockGetMatchSingle = vi.fn(() => ({
			data: mockCurrentMatchData,
			error: null,
		}));
		const mockGetMatchEq = vi.fn(() => ({ single: mockGetMatchSingle }));
		const mockGetMatchSelect = vi.fn(() => ({ eq: mockGetMatchEq }));
		...
		const mockPlayerSelect = vi.fn(() => ({
			eq: vi.fn(() => ({
				single: vi.fn(() => ({
					data: { matches: 1, wins: 0, losses: 0, goals: 0, assists: 0 },
					error: null,
				})),
			})),
		}));
		...
		const match = {
			...
			goals: [
				{ playerId: "1", minute: 5 },
				{ playerId: "3", minute: 30, assistById: "1" },
			],
		};

		await expect(service.editMatch(match)).resolves.toBeUndefined();
		expect(mockInsertPlayers).toHaveBeenCalledWith([
			{ match_id: "match-id", player_id: "1", team: "A" },
			{ match_id: "match-id", player_id: "3", team: "B" },
		]);
		expect(mockInsertGoals).toHaveBeenCalledWith([
			{ match_id: "match-id", player_id: "1", assist_by_id: null, minute: 5 },
			{ match_id: "match-id", player_id: "3", assist_by_id: "1", minute: 30 },
		]);
	});
